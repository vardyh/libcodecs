...
{
  HRESULT hr;
  hr = m_ExtractCallback->SetOperationResult(m_RealOutStream, resOp);
...
HRESULT CFolderOutStream::WriteEmptyFiles()
{
  ...
  for (; m_CurrentIndex < m_ExtractStatuses->Size(); m_CurrentIndex++)
  {
    ...
    RINOK(result);
    result = m_ExtractCallback->SetOperationResult(m_RealOutStream, NExtract::NOperationResult::kOK);
    m_RealOutStream.Release();
    RINOK(result);
  }
  return S_OK;
...
HRESULT CFolderOutStream::Unsupported()
{
  while(m_CurrentIndex < m_ExtractStatuses->Size())
  {
    ...
    if (result != S_FALSE && result != S_OK)
      return result;
    result = m_ExtractCallback->SetOperationResult(m_RealOutStream, NExtract::NOperationResult::kUnSupportedMethod);
    m_RealOutStream.Release();
    RINOK(result);
    m_CurrentIndex++;
  }
    ...
    Int32 testModeSpec, IArchiveExtractCallback *extractCallback)
{
  ...
  for(i = 0; i < numItems;)
  {
    ...
    if (item.IsDir())
    {
      ...
      HRESULT hr = extractCallback->SetOperationResult(realOutStream, NExtract::NOperationResult::kOK);
    ...
    if (folderIndex < 0)
    {
      ...
      RINOK(extractCallback->GetStream(index, &realOutStream, askMode));
      RINOK(extractCallback->PrepareOperation(askMode));
      HRESULT hr = extractCallback->SetOperationResult(realOutStream, NExtract::NOperationResult::kDataError);
      realOutStream.Release();
      RINOK(hr);
      continue;
    }
