    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  if (result == S_OK) {
    result = extractCallback->SetOperationResult(
	    outStreamSpec->_stream, NExtract::NOperationResult::kOK);
  } else if (result == S_FALSE) {
    result = extractCallback->SetOperationResult(
	    outStreamSpec->_stream, NExtract::NOperationResult::kDataError);
  }
...
static IInArchive *CreateArc() { return new NArchive::NBz2::CHandler; }
#ifndef EXTRACT_ONLY
static IOutArchive *CreateArcOut() { return new NArchive::NBz2::CHandler; }
#else
#define CreateArcOut 0
#endif

static CArcInfo g_ArcInfo =
{ ".bz2", { 'B', 'Z', 'h' }, 3, CreateArc, CreateArcOut };
