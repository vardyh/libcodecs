...
HRESULT CChmFolderOutStream::WriteEmptyFiles()
{
  ...
  for (;m_CurrentIndex < m_NumFiles; m_CurrentIndex++)
  {
    ...
    if (result == S_OK) {
	    result = m_ExtractCallback->SetOperationResult(
		    m_RealOutStream, NExtract::NOperationResult::kOK);
    }
    m_RealOutStream.Release();
    RINOK(result);
  }
...
HRESULT CChmFolderOutStream::Write2(const void *data, UInt32 size, UInt32 *processedSize, bool isOK)
{
  ...
  while(size != 0)
  {
    if (m_FileIsOpen)
    {
      ...
      {
       HRESULT hr = m_ExtractCallback->SetOperationResult(
	       m_RealOutStream,
	       m_IsOk ?
    ...
    Int32 testModeSpec, IArchiveExtractCallback *extractCallback)
{
  ...
  if (m_Database.LowLevel)
  {
    ...
    for (i = 0; i < numItems; i++, currentTotalSize += currentItemSize)
    {
      ...
      if (m_Database.NewFormat)
      {
        ...
        RINOK(extractCallback->SetOperationResult(realOutStream, NExtract::NOperationResult::kOK));
        continue;
      }
      ...
      if (item.Section != 0)
      {
	RINOK(extractCallback->SetOperationResult(realOutStream, NExtract::NOperationResult::kUnSupportedMethod));
        continue;
      }
	...
	RINOK(extractCallback->SetOperationResult(realOutStream, NExtract::NOperationResult::kOK));
      ...
      HRESULT hr = extractCallback->SetOperationResult(
	      realOutStream,
	      (copyCoderSpec->TotalSize == item.Size) ?
	      NExtract::NOperationResult::kOK:
	      NExtract::NOperationResult::kDataError);
  ...
  for (i = 0; i < numItems;)
  {
    ...
    if (item.IsDir())
    {
      ...
      RINOK(extractCallback->PrepareOperation(askMode));
      HRESULT hr = extractCallback->SetOperationResult(
	      realOutStream, NExtract::NOperationResult::kOK);
      realOutStream.Release();
      RINOK(hr);
      continue;
    }
    ...
    if (item.Size == 0 || sectionIndex == 0)
    {
      ...
      HRESULT hr = extractCallback->SetOperationResult(realOutStream, opRes);
      realOutStream.Release();
      RINOK(hr);
      currentTotalSize += item.Size;
      continue;
